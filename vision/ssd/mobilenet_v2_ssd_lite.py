...
def create_mobilenetv2_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    output_times = int(len(config.specs) /6.0)
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=int(round(576 * width_mult)), out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
    ...
    classification_headers = ModuleList([
        SeperableConv2d(in_channels=int(round(576 * width_mult)), out_channels=6 * num_classes, kernel_size=3, padding=1),
