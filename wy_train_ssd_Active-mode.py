...
from pilot_test.sample_method import random_sampling, sequential_sampling, uncertainty_sampling, uncertainty_modify_sampling
import time 
# import pynvml
# pynvml.nvmlInit()
...
def Argments():
    ...
    parser.add_argument('--sample_method', type = str, default = 'random', help= "random, sequential, uncertainty")
...
def main_acitve_mode(args):
    ...
    # Model setting
    creat_net, config = Load_model_configuration(args)
    ...
    active_dataset = active_dataset_setting()

    active_dataset.Active_mode()    

    # labeled, unlabeled = val_loader.dataset.dataset_information()
    # index_list = np.random.permutation(unlabeled[-1])
    # k = 6
    # k_folder_counter = 0
    # k_folder_validation = [index_list[i * unlabeled[-1]//k:(i+1)*unlabeled[-1]//k].tolist() for i in range(k) ]
    _crossvalidation_handler =CrossValidation(val_loader,k_folder = k)
    ...
    for q_iter in range(Query_iteration):
        ...
        elif _setting_sampler == "sequential":
            Sampling_method = sequential_sampling
            query_index = unlabeled[:query_item]
            train_loader.dataset.setting_be_selected_sample(query_index)
            active_dataset.setting_be_selected_sample(query_index)
        elif _setting_sampler == "uncertainty_modify":
            Sampling_method = uncertainty_modify_sampling
        ...
        for epoch in range(0, args['flow_control']['num_epochs']):
            ...
            if epoch % args['flow_control']['validation_epochs'] == 0 or epoch == args['flow_control']['num_epochs'] - 1:
                val_loader = _crossvalidation_handler()
                # val_loader.dataset.reset_label_unlabel_set()
                # val_loader.dataset.setting_be_selected_sample(k_folder_validation[k_folder_counter%k])
                # k_folder_counter += 1
                # val_loader.dataset.training_mode()
...
def dataset_loading(args,config):    
    ...
    if dataset_name == "voc":
        ...
        #                                 transform=test_transform, target_transform=target_transform)
        val_dataset = ECP_table_comm( args["Datasets"]["ecp"]["val_image_path"], 
                                   args["Datasets"]["ecp"]["val_anno_path"], 
                                   transform=train_transform, target_transform=target_transform)
        dataset.Active_mode()
    elif dataset_name in ['virat', 'virat_seq']:
        val_dataset = VIRAT_table_comm( args["Datasets"]["virat_seq"]["train_image_path"],
                                     args["Datasets"]["virat_seq"]["train_anno_path"],
                                     transform=test_transform, target_transform=target_transform, downpurning_ratio = 0.2 * 3. / 9.)
        dataset.Active_mode()
    # elif dataset_name = "ecp-random":
...
class CrossValidation(object):
    def __init__(self, val_loader, k_folder = 10):
        
        self.val_loader = val_loader
        self.k_folder = k_folder
        self.k_folder_counter = 0

        labeled, unlabeled = val_loader.dataset.dataset_information()
        index_list = np.random.permutation(unlabeled[-1])
        k = k_folder
        self.val_loader.Active_mode()
        self.k_folder_validation = [index_list[i * unlabeled[-1]//k:(i+1)*unlabeled[-1]//k].tolist() for i in range(k) ]
    def __call__(self):
        self.val_loader.dataset.reset_label_unlabel_set()
        self.val_loader.dataset.setting_be_selected_sample(self.k_folder_validation[self.k_folder_counter%self.k_folder])
        self.k_folder_counter += 1
        self.val_loader.dataset.training_mode()
        if self.k_folder_counter % self.k_folder and self.k_folder_counter == self.k_folder * 1000:
            self.k_folder_counter = 0
