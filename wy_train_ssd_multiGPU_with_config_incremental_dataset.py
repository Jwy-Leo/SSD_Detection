...
def Argments():
    ...
    parser.add_argument('--sample_method', type = str, default = 'random', help= "random, uncertainty")
...
def main_acitve_mode(args):
    ...
    for q_iter in range(Query_iteration):
        ...
        if _setting_sampler == "random":
            ...
            # batch = torch.stack(imgs_list)
            net.train(False)
            # memory_item = (memory_info() * 4 - 1024 * 1024 * 1024 * 4) /4
            # max_num = int( float(memory_item) / float(batch[0,...].view(-1).shape[0]) )
            max_num = 50
            confidences = []
            for index in range(len(imgs_list)//max_num+1):
                with torch.no_grad():
                    ...
                    # sub_batch = batch[begin_pointer:end_pointer,...].cuda()
